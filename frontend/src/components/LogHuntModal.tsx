...
export default function LogHuntModal({ lat, lon, onClose, onSuccess, embedded = false }: LogHuntModalProps) {
    ...
    try {
      ...
      try {
        ...
        if (badgeResult.newly_earned.length > 0) {
          setNewBadges(badgeResult.newly_earned);
  ...
  return (
    <>
    <div className={embedded ? '' : 'fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm'}>
      {/* Badge earned notification */}
      {newBadges.length > 0 && (
        <div className="bg-white border border-stone-200 rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-sm flex flex-col items-center px-6 py-8 gap-4">
          <div className="text-3xl">🎉</div>
          <h2 className="text-stone-900 font-bold text-lg text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 my-2">
            {newBadges.map((b) => (
              <div key={b.id} className="flex flex-col items-center gap-1">
                <BadgeDisplay badge={b} earned size="lg" />
                <span className="text-stone-700 text-xs font-medium">{b.name}</span>
                <span className="text-amber-600 text-xs">+{b.points} pts</span>
              </div>
            ))}
          </div>
          <button
            onClick={() => { onSuccess(); onClose(); }}
            className="w-full px-5 py-2.5 text-sm font-medium text-white bg-stone-800 hover:bg-stone-700 rounded-xl transition-colors"
          >
            Awesome!
          </button>
        </div>
      ...
      <div className="bg-white border border-stone-200 rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-md h-[100dvh] sm:h-auto flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200">
          ...
          <button
            onClick={onClose}
            className="text-stone-400 hover:text-stone-700 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center"
            aria-label="Close"
          >
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-4 sm:px-6 py-4 border-t border-stone-200">
          <button
            type="button"
            onClick={onClose}
            className="px-4 min-h-[44px] text-sm text-stone-500 hover:text-stone-800 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-5 min-h-[44px] text-sm font-medium text-white bg-stone-800 hover:bg-stone-700 disabled:opacity-40 disabled:cursor-not-allowed rounded-xl transition-colors flex items-center gap-2"
          >
            {isSubmitting && (
