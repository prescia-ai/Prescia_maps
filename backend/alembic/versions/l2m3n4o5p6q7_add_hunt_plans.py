...
def upgrade() -> None:
    ...
    op.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'hunt_plan_status_enum') THEN
    ...
    if not inspector.has_table("hunt_plans"):
        op.create_table(
            "hunt_plans",
            sa.Column(
                "id",
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE IF EXISTS hunt_plan_status_enum")
