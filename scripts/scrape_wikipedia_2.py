#!/usr/bin/env python3
"""
Script to scrape Wikipedia for additional historical events and store in database.
Run from project root: python scripts/scrape_wikipedia_2.py

This script is structurally identical to scripts/scrape_wikipedia.py but
imports from app.scrapers.wikipedia_2 to cover URLs not included in the
first script (additional ghost town states, old railroads, extra stagecoach
routes, historic settlements, gold-rush sites, mines, trails, battles,
ferries, cemeteries, missions, and shipwrecks).
...
from app.scrapers.wikipedia_2 import WIKIPEDIA_PAGES_2, scrape_source_2
...
async def main():
    parser = argparse.ArgumentParser(description="Scrape Wikipedia (part 2) for historical events and store in database.")
    parser.add_argument(
        "--no-geocode",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--checkpoint",
        default="scrape_checkpoint_2.json",
        metavar="PATH",
        help="Path to checkpoint file for crash-safe resume (default: scrape_checkpoint_2.json).",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
    ...
    total_sources = len(WIKIPEDIA_PAGES_2)
    ...
    for page_config in WIKIPEDIA_PAGES_2:
        ...
        try:
            events = await scrape_source_2(page_config, geocode_missing=not args.no_geocode)
